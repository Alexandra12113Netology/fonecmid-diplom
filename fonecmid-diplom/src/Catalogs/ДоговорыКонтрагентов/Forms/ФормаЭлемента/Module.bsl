...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    // ВКМ 11.04.2026 Добавление полей реквизитов
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// ВКМ 11.04.2026 Добавление
	Если ЗначениеЗаполнено(Объект.ВКМ_ПериодС) Тогда
		ТекущийОбъект.ВКМ_ПериодС = Объект.ВКМ_ПериодС;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ВКМ_ПериодПо) Тогда
		ТекущийОбъект.ВКМ_ПериодПо = Объект.ВКМ_ПериодПо;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ВКМ_СтоимостьЧасаРаботы) Тогда
		ТекущийОбъект.ВКМ_СтоимостьЧасаРаботы = Объект.ВКМ_СтоимостьЧасаРаботы;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.ВКМ_АбонентскаяПлата) Тогда
		ТекущийОбъект.ВКМ_АбонентскаяПлата = Объект.ВКМ_АбонентскаяПлата;
	КонецЕсли;
	// ВКМ Конец добавления

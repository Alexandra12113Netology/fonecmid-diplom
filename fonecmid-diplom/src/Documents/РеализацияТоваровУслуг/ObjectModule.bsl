...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Движения.ВКМ_ВыставленныеКОплатеРаботы.Очистить();
	...
	Если Движения.ВКМ_ВыставленныеКОплатеРаботы.Количество() = 0 Тогда
   
   	Сообщить("Внимание! Движения по регистру ВКМ_ВыставленныеКОплатеРаботы НЕ созданы!");
   	Иначе
  	Сообщить("Успех: создано записей: " + Строка(Движения.ВКМ_ВыставленныеКОплатеРаботы.Количество()));
	КонецЕсли;
	
	Попытка
    Движения.ВКМ_ВыставленныеКОплатеРаботы.Записать();
    Исключение
...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт
    ...
    Запрос.Текст = 
	 ...
	 |		Договор.ВидДоговора = &АбонентскоеОбслуживание) КАК ВКМ_ВыполненныеКлиентуРаботыОбороты";

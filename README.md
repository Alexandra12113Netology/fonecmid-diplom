...
6. Создать договоры с видом Абонентское обслуживание
...
14. Зайти под пользователем Специалист и создать документ обслуживание клиента. После записи поступит оповещение в телеграм
...
22. Создать документ график отпусков, при привышении 28 дней строка выделится желтым
...
В конфигураторе
- у пользователей снять галочку "Защита от опасных действий", добавить роли "Администратор системы" и "Добавление и изменение папок и файлов" В режиме предприятия
- добавить права доступа "Открытие внешних отчетов и обработок"

Запуск автотестов:
Скачайте Vanessa Automation из официального git-репозитория. Загрузите готовую базу из файла с расширением *.dt выбор Клиента тестирования Имя - DiplomУправлениеITФирмой Тип клиента - Тонкий Прописать путь к информационной базе (в зависимости от расположения базы TestСlient)

Доп. параметры в зависимости от выбора пользователя по сценарию: Тест ОбслуживаниеКлиента запускать от имени пользователей - /N"ПетровВС(Специалист)" и /N"НовиковДЛ(Специалист)" Тест ЗакрытиеОбслуживанийСпециалист запускать от имени пользователей - /N"ПетровВС(Специалист)" и /N"НовиковДЛ(Специалист)" Тест МассовоеСозданиеАктов запускать от имени пользователя /N"ПоляковаИА(Бухгалтер)" Тест ОтчетАнализВыставленныхАктов запускать от имени пользователя /N"ПоляковаИА(Бухгалтер)"

Порядок отработки сценариев: запуск каждого отдельно друг от друга в следующем порядке:

Планирование нескольких обслуживаний на специалистов от имени пользователей Менеджер
Закрытие обслуживаний от имени пользователей Специалист
Массовое создание актов от имени пользователя Бухгалтер
Формирование отчёта Анализ выставленных актов от имени пользователя Бухгалтер

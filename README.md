...
14. Зайти под пользователем Менеджер и создать документ обслуживание клиента. После записи поступит оповещение в телеграм
...
Доп. параметры в зависимости от выбора пользователя по сценарию: Тест ОбслуживаниеКлиента запускать от имени пользователей - /N"СоколовВВ (МенеджерЗакупки)" Тест ЗакрытиеОбслуживанийСпециалист запускать от имени пользователей - /N"ПетровВС(Специалист)" и /N"НовиковДЛ(Специалист)" Тест МассовоеСозданиеАктов запускать от имени пользователя /N"ПоляковаИА(Бухгалтер)" Тест ОтчетАнализВыставленныхАктов запускать от имени пользователя /N"ПоляковаИА(Бухгалтер)"
